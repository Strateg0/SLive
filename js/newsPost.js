...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (!container) {
    console.error('Container #news-posts not found');
    return;
  }

  // WordPress REST API URL with ?rest_route=
  const apiURL = `https://strategma.bg/content/?rest_route=/wp/v2/posts&categories=${newsCategoryID}&per_page=5&_embed`;
    ...
    .then(data => {
      ...
      posts.forEach(post => {
        ...
        if (post._embedded?.['wp:term']?.[0]) {
          const foundCat = post._embedded['wp:term'][0].find(cat => cat.id === newsCategoryID);
